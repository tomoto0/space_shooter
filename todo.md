...
## Wave 6 Background and Design Adjustments
- [x] Wave 6の背景をPC全画面に広げる（地球を大きな球体として配置）
- [x] 地球を画面下部5分の1に配置（背景メインは宇宙空間）
- [x] 機体デザインを白い三角形の丸みを帯びたデザインに維持
- [x] 三つの青白いエンジンが見えるデザインを維持
- [x] 隗石の数を2倍に増やす（asteroidChanceを0.15から0.30に）
- [x] 隗石のスピードを少し上げる（3.5倍から4倍に）
- [x] 機体のウィングが左右対称に見えるようにライティングを調整（左右対称のライトを追加）
