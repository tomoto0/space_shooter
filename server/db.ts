...
import { InsertUser, users, gameScores, InsertGameScore } from "../drizzle/schema";
...
export async function saveGameScore(score: InsertGameScore) {
  ...
  try {
    ...
    console.error("[Database] Failed to save score:", error);
    throw error;
  }
}

/**
 * Get top scores for ranking (limit 100)
 */
export async function getTopScores(limit: number = 100) {
  ...
  try {
    ...
    console.error("[Database] Failed to get scores:", error);
    return [];
  }
}
